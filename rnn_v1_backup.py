...
def batch_creator(batch_size, dataset_length, timesteps):
    ...
    offset_rand = random.randrange(0, dataset_length - timesteps - batch_size)
    for i in range(batch_size):
        #offset = random.randrange(0, dataset_length - timesteps)
...
with tf.Session() as sess:
    ...
    for offset in range(X_val.shape[0] - timesteps - pred_timesteps):
        ...
        for i in range(pred_timesteps):
            ...
            loss, pred_y = sess.run([cost, pred], feed_dict={x: val_x, y: val_y})
            pred_y = pred_y[0][0]
