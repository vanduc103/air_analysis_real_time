...
batch_size = 128
display_step = 1000
model_path = "model/rnn_aqi_model_v2.ckpt"
output_path = "model/output2.txt"
...
# Metric
mae = tf.metrics.mean_absolute_error(pred, y) 
...
with tf.Session() as sess:
    ...
    for offset in range(X_val.shape[0] - timesteps - pred_timesteps):
        ...
        for i in range(pred_timesteps):
            ...
            pred_y = sess.run([pred], feed_dict={x: val_x, x_w: val_x_w, y: val_y})
            mae_metric = sess.run([mae], feed_dict={x: val_x, x_w: val_x_w, y: val_y})
            pred_y = pred_y[0][0]
            #print("{:.2f}".format(pred_y) + "      " + str(val_y))
            val_X_base = np.append(val_X_base, pred_y)
            mae_val += mae_metric

    # Print validate error
    print("Validate MAE - {:.6f}".format(loss_val/val_steps))
    outfile.write("Validate MAE - {:.6f}\n".format(mae_val/val_steps))
