...
void AddDot4x4( int, float *, int, float *, int, float *, int, int * );

void PackMatrixA( int, float *, int, float *, int * );
                                       ...
                                       float *c, int ldc )
{
  ...
  int
    *kIdx = ( int * ) malloc( (m*k) * sizeof( int ) );
  ...
  for ( j=0; j<n; j+=NR ) {        /* Loop over the columns of C, unrolled by 4 */
    ...
    for ( i=0; i<m; i+=MR ) {        /* Loop over the rows of C */
	 ...
	 one routine (four inner products) */
      if ( j == 0 ) {
        PackMatrixA( k, &A( i, 0 ), MR, &packedA[ i*k ], &kIdx[ i*k ] );
      }
      
      AddDot4x4( k, &packedA[ i*k ], lda, &packedB[ k*j ], ldb, &C( i,j ), ldc, &kIdx[ i*k ] );
    }
  }
}

void PackMatrixA( int k, float *a, int lda, float *a_to, int *k_idx )
{
  int j, i;
...
void AddDot4x4( int k, float *a, int lda, float *b, int ldb, float *c, int ldc, int *k_idx )
{
